...
Assumptions made are the ambulance is traveling at a spped of 90 kmph
File input format
number of nodes
...
void dijkstra_amb_to_hosp(status **arr, int n, int src)
{
    ...
    if (hosp_loc == 4)
    {
        ...
        if (num1 < 1)
        {
            printf("\n\nNO seats are available in the nearest hospital and the next nearest hospital is at location 3\n\n");
            exit(0);
        }

        printf("\n\nThe seats available in the hospital are :- %d ", num1);
    ...
    else
    {
        ...
        if (num1 < 1)
        {
            printf("\n\nNO seats are available in the nearest hospital and the next nearest hospital is at location 4\n\n");
            exit(0);
        }
...
void mark_faulty(status **arr, int n, int amb_number)
{
    for (int i = 0; i < n; i++)
    {
        arr[i][amb_number].a = -1;
    }
}
...
int main()
{
    ...
    // here we are creating an adjacency matrix of s0tatus data type
    ...
    printf("\nEnter patient's location : \n");
    scanf("%d", &pat_loc);
    // pat_loc=rand()%n;
    printf("\nPatient's Location detected at location %d a/c to SOS msg gps.\n\n", pat_loc);
    for (int i = 0; i < n; i++)
    {
        arr[i][pat_loc].a = 0;
        arr[i][pat_loc].h = 0;
    ...
    int f;
    printf("Please enter the faulty ambulance number : ");
    scanf("%d", &f);
    if (f < n)
    {
        mark_faulty(arr, n, f);
        printf("\nAmbulance %d is marked faulty.", f);
    }
    else
    {
        printf("\nNot a valid ambulance number.\n");
    }
